...
class PoseRTV {
  PoseRTV();
  PoseRTV(const gtsam::Vector& rtv);
  PoseRTV(const gtsam::Point3& pt, const gtsam::Rot3& rot,
          const gtsam::Vector& vel);
  PoseRTV(const gtsam::Rot3& rot, const gtsam::Point3& pt,
          const gtsam::Vector& vel);
  ...
  static size_t Dim();
  size_t dim() const;
  gtsam::PoseRTV retract(const gtsam::Vector& v) const;
  gtsam::Vector localCoordinates(const gtsam::PoseRTV& p) const;
  static gtsam::PoseRTV Expmap(const gtsam::Vector& v);
  ...
  // IMU/dynamics
  gtsam::PoseRTV planarDynamics(double vel_rate, double heading_rate,
                                double max_accel, double dt) const;
  gtsam::PoseRTV flyingDynamics(double pitch_rate, double heading_rate,
                                double lift_control, double dt) const;
  gtsam::PoseRTV generalDynamics(const gtsam::Vector& accel,
                                 const gtsam::Vector& gyro, double dt) const;
  Vector imuPrediction(const gtsam::PoseRTV& x2, double dt) const;
  gtsam::Point3 translationIntegration(const gtsam::PoseRTV& x2,
                                       double dt) const;
...
class Pose3Upright {
  ...
  size_t dim() const;
  gtsam::Pose3Upright retract(const gtsam::Vector& v) const;
  ...
  static gtsam::Pose3Upright Expmap(const gtsam::Vector& xi);
...
class BearingS2 {
  ...
  static gtsam::BearingS2 fromDownwardsObservation(const gtsam::Pose3& A,
                                                   const gtsam::Point3& B);
  static gtsam::BearingS2 fromForwardObservation(const gtsam::Pose3& A,
                                                 const gtsam::Point3& B);
...
class SimWall2D {
  ...
  //   bool intersects(const gtsam::SimWall2D& wall, gtsam::Point2* pt =
  //   nullptr) const;

  gtsam::Point2 norm() const;
  gtsam::Rot2 reflection(const gtsam::Point2& init,
                         const gtsam::Point2& intersection) const;
};
...
class SimPolygon2D {
  ...
  static gtsam::SimPolygon2D createRectangle(const gtsam::Point2& p,
                                             double height, double width);

  static gtsam::SimPolygon2D randomTriangle(
      double side_len, double mean_side_len, double sigma_side_len,
      double min_vertex_dist, double min_side_len,
      const gtsam::SimPolygon2DVector& existing_polys);
  static gtsam::SimPolygon2D randomRectangle(
      double side_len, double mean_side_len, double sigma_side_len,
      double min_vertex_dist, double min_side_len,
      const gtsam::SimPolygon2DVector& existing_polys);
  ...
  static bool anyContains(const gtsam::Point2& p,
                          const gtsam::SimPolygon2DVector& obstacles);
  static bool anyOverlaps(const gtsam::SimPolygon2D& p,
                          const gtsam::SimPolygon2DVector& obstacles);
  static bool insideBox(double s, const gtsam::Point2& p);
  static bool nearExisting(const gtsam::Point2Vector& S, const gtsam::Point2& p,
                           double threshold);
  ...
  static gtsam::Point2 randomBoundedPoint2(
      double boundary_size, const gtsam::Point2Vector& landmarks,
      const gtsam::SimPolygon2DVector& obstacles, double min_landmark_dist);
  static gtsam::Point2 randomBoundedPoint2(
      double boundary_size, const gtsam::SimPolygon2DVector& obstacles);
  static gtsam::Point2 randomBoundedPoint2(
      const gtsam::Point2& LL_corner, const gtsam::Point2& UR_corner,
      const gtsam::Point2Vector& landmarks,
      const gtsam::SimPolygon2DVector& obstacles, double min_landmark_dist);
  static gtsam::Pose2 randomFreePose(
      double boundary_size, const gtsam::SimPolygon2DVector& obstacles);
};
...
virtual class BetweenFactorEM : gtsam::NonlinearFactor {
  ...
  void updateNoiseModels_givenCovs(const gtsam::Values& values,
                                   const gtsam::Matrix& cov1,
                                   const gtsam::Matrix& cov2,
                                   const gtsam::Matrix& cov12);
...
virtual class TransformBtwRobotsUnaryFactorEM : gtsam::NonlinearFactor {
  TransformBtwRobotsUnaryFactorEM(
      size_t key, const T& relativePose, size_t keyA, size_t keyB,
      const gtsam::Values& valA, const gtsam::Values& valB,
      const gtsam::noiseModel::Gaussian* model_inlier,
      const gtsam::noiseModel::Gaussian* model_outlier, double prior_inlier,
  ...
  TransformBtwRobotsUnaryFactorEM(
      ...
      const gtsam::noiseModel::Gaussian* model_inlier,
      const gtsam::noiseModel::Gaussian* model_outlier, double prior_inlier,
      double prior_outlier, bool flag_bump_up_near_zero_probs,
...
virtual class TransformBtwRobotsUnaryFactor : gtsam::NonlinearFactor {
  TransformBtwRobotsUnaryFactor(size_t key, const T& relativePose, size_t keyA,
                                size_t keyB, const gtsam::Values& valA,
...
virtual class TOAFactor : gtsam::NonlinearFactor {
  // For now, because of overload issues, we only expose constructor with known
  // sensor coordinates:
  TOAFactor(size_t key1, gtsam::Point3 sensor, double measured,
            const gtsam::noiseModel::Base* noiseModel);
  static void InsertEvent(size_t key, const gtsam::Event& event,
                          gtsam::Values* values);
};
...
virtual class IMUFactor : gtsam::NoiseModelFactor {
  ...
  IMUFactor(const gtsam::Vector& accel, const gtsam::Vector& gyro, double dt,
            size_t key1, size_t key2, const gtsam::noiseModel::Base* model);

  /** Full IMU vector specification */
  IMUFactor(const gtsam::Vector& imu_vector, double dt, size_t key1,
            size_t key2, const gtsam::noiseModel::Base* model);
...
virtual class FullIMUFactor : gtsam::NoiseModelFactor {
  /** Standard constructor */
  FullIMUFactor(const gtsam::Vector& accel, const gtsam::Vector& gyro,
                double dt, size_t key1, size_t key2,
                const gtsam::noiseModel::Base* model);

  /** Single IMU vector - imu = [accel, gyro] */
  FullIMUFactor(const gtsam::Vector& imu, double dt, size_t key1, size_t key2,
                const gtsam::noiseModel::Base* model);
...
virtual class PendulumFactor1 : gtsam::NonlinearFactor {
  ...
  Vector evaluateError(const double& qk1, const double& qk,
                       const double& v) const;
};
...
virtual class PendulumFactor2 : gtsam::NonlinearFactor {
  /** Standard constructor */
  PendulumFactor2(size_t vk1, size_t vk, size_t qKey, double dt, double L,
                  double g);

  Vector evaluateError(const double& vk1, const double& vk,
                       const double& q) const;
};

virtual class PendulumFactorPk : gtsam::NonlinearFactor {
  /** Standard constructor */
  PendulumFactorPk(size_t pk, size_t qk, size_t qk1, double h, double m,
                   double r, double g, double alpha);

  Vector evaluateError(const double& pk, const double& qk,
                       const double& qk1) const;
};

virtual class PendulumFactorPk1 : gtsam::NonlinearFactor {
  /** Standard constructor */
  PendulumFactorPk1(size_t pk1, size_t qk, size_t qk1, double h, double m,
                    double r, double g, double alpha);

  Vector evaluateError(const double& pk1, const double& qk,
                       const double& qk1) const;
};
...
virtual class Reconstruction : gtsam::NoiseModelFactor {
  ...
  Vector evaluateError(const gtsam::Pose3& gK1, const gtsam::Pose3& gK,
                       const gtsam::Vector& xiK) const;
};
...
virtual class DiscreteEulerPoincareHelicopter : gtsam::NoiseModelFactor {
  DiscreteEulerPoincareHelicopter(size_t xiKey, size_t xiKey_1, size_t gKey,
                                  double h, const gtsam::Matrix& Inertia,
                                  const gtsam::Vector& Fu, double m);

  Vector evaluateError(const gtsam::Vector& xiK, const gtsam::Vector& xiK_1,
...
void synchronize(gtsam::ConcurrentFilter& filter,
                 gtsam::ConcurrentSmoother& smoother);
...
virtual class ConcurrentBatchFilter : gtsam::ConcurrentFilter {
  ...
  gtsam::ConcurrentBatchFilterResult update(
      const gtsam::NonlinearFactorGraph& newFactors,
      const gtsam::Values& newTheta, const gtsam::KeyList& keysToMove);
...
virtual class InvDepthFactorVariant1 : gtsam::NoiseModelFactor {
  InvDepthFactorVariant1(size_t poseKey, size_t landmarkKey,
                         const gtsam::Point2& measured, const gtsam::Cal3_S2* K,
                         const gtsam::noiseModel::Base* model);
};

#include <gtsam_unstable/slam/InvDepthFactorVariant2.h>
virtual class InvDepthFactorVariant2 : gtsam::NoiseModelFactor {
  InvDepthFactorVariant2(size_t poseKey, size_t landmarkKey,
                         const gtsam::Point2& measured, const gtsam::Cal3_S2* K,
...
virtual class InvDepthFactorVariant3a : gtsam::NoiseModelFactor {
  InvDepthFactorVariant3a(size_t poseKey, size_t landmarkKey,
                          const gtsam::Point2& measured,
                          const gtsam::Cal3_S2* K,
                          const gtsam::noiseModel::Base* model);
};
virtual class InvDepthFactorVariant3b : gtsam::NoiseModelFactor {
  InvDepthFactorVariant3b(size_t poseKey1, size_t poseKey2, size_t landmarkKey,
                          const gtsam::Point2& measured,
                          const gtsam::Cal3_S2* K,
                          const gtsam::noiseModel::Base* model);
};
...
class Mechanization_bRn2 {
  Mechanization_bRn2();
  Mechanization_bRn2(gtsam::Rot3& initial_bRn, const gtsam::Vector& initial_x_g,
                     const gtsam::Vector& initial_x_a);
  gtsam::Vector b_g(double g_e);
  gtsam::Rot3 bRn();
  gtsam::Vector x_g();
  gtsam::Vector x_a();
  static gtsam::Mechanization_bRn2 initialize(const gtsam::Matrix& U,
                                              const gtsam::Matrix& F,
                                              double g_e);
...
class AHRS {
  ...
  pair<gtsam::Mechanization_bRn2, gtsam::GaussianDensity*> initialize(
      double g_e);
  pair<gtsam::Mechanization_bRn2, gtsam::GaussianDensity*> integrate(
      ...
      const gtsam::Vector& u, double dt);
  pair<gtsam::Mechanization_bRn2, gtsam::GaussianDensity*> aid(
      const gtsam::Mechanization_bRn2& mech, gtsam::GaussianDensity* state,
      const gtsam::Vector& f, bool Farrel);
...
virtual class ProjectionFactorPPP : gtsam::NoiseModelFactor {
  ProjectionFactorPPP(const gtsam::Point2& measured,
                      ...
                      size_t transformKey, size_t pointKey,
                      const CALIBRATION* k);

  ProjectionFactorPPP(const gtsam::Point2& measured,
                      const gtsam::noiseModel::Base* noiseModel, size_t poseKey,
                      size_t transformKey, size_t pointKey,
                      const CALIBRATION* k, bool throwCheirality,
...
typedef gtsam::ProjectionFactorPPP<gtsam::Pose3, gtsam::Point3,
                                   gtsam::Cal3Fisheye>
...
virtual class ProjectionFactorPPPC : gtsam::NoiseModelFactor {
  ProjectionFactorPPPC(const gtsam::Point2& measured,
                       const gtsam::noiseModel::Base* noiseModel,
                       size_t poseKey, size_t transformKey, size_t pointKey,
                       size_t calibKey);

  ProjectionFactorPPPC(const gtsam::Point2& measured,
                       const gtsam::noiseModel::Base* noiseModel,
                       size_t poseKey, size_t transformKey, size_t pointKey,
                       size_t calibKey, bool throwCheirality,
                       bool verboseCheirality);
...
typedef gtsam::ProjectionFactorPPPC<gtsam::Pose3, gtsam::Point3,
                                    gtsam::Cal3Fisheye>
    ProjectionFactorPPPCCal3Fisheye;

#include <gtsam_unstable/slam/ProjectionFactorRollingShutter.h>
virtual class ProjectionFactorRollingShutter : gtsam::NoiseModelFactor {
  ProjectionFactorRollingShutter(const gtsam::Point2& measured, double alpha,
                                 const gtsam::noiseModel::Base* noiseModel,
                                 size_t poseKey_a, size_t poseKey_b,
                                 size_t pointKey, const gtsam::Cal3_S2* K);
